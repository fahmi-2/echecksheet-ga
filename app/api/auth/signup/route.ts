...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO users (
        id, username, full_name, nik, department, role, password_hash, checksheets
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8::text[])
      RETURNING id, username, full_name, nik, department, role, checksheets`,
      [userId, username.trim(), fullName.trim(), nik.trim(), department, role, hashedPassword, validChecksheets]
    );
