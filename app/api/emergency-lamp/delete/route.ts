  ...
  try {
    ...
    try {
      ...
      const [items] = await connection.query(
        `SELECT foto FROM emergency_lamp_items WHERE record_id = ? AND foto IS NOT NULL`,
        [id]
      );
      ...
      const [result] = await connection.query(
        `DELETE FROM emergency_lamp_records WHERE id = ?`,
        [id]
      );
    ...
    console.error('Delete emergency lamp error:', error);
    return NextResponse.json(
      { 
        success: false, 
