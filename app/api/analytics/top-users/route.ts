...
export async function GET(request: NextRequest) {
  try {
    ...
    function queryGA(gaSlug: string) {
      return run(`
        SELECT
          COALESCE(h.submitted_by, h.checker, h.inspector_name) AS name,
        ...
        WHERE t.slug = $3 AND h.check_date BETWEEN $1 AND $2
          AND COALESCE(h.submitted_by, h.checker, h.inspector_name) IS NOT NULL
          AND COALESCE(h.submitted_by, h.checker, h.inspector_name) <> ''
        GROUP BY COALESCE(h.submitted_by, h.checker, h.inspector_name)
