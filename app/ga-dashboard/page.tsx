...
export default function GADashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (!formConfig) throw new Error('Konfigurasi form tidak ditemukan');
      ...
      const [analyticsResult, topUsersResult, historyResult] = await Promise.allSettled([
        ...
        fetchHistory('/analytics/history', formConfig.slug,
          selectedForm !== 'All Category' ? selectedForm : undefined,
          itemsPerPage, analyticsParams.dateFrom, analyticsParams.dateTo, currentPage)
      ]);
      ...
      setDashboardData({ stats: { total: 0, completed: 0, pending: 0, completionRate: '0.0' }, trendData: [], distributionData: [], topUsers: [], historyData: [] });
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    const cleanup = onDashboardRefresh((formType?: string) => {
      if (shouldRefreshForForm(selectedForm, formType || undefined)) loadDashboardData();
    }, { debounceMs: 500 });
    return cleanup;
  ...
  const distributionChartData = useMemo(() => {
    ...
    dashboardData.distributionData.forEach(item => {
      ...
      if (item.status === 'OK') current.ok += item.count;
      else if (item.status === 'NG') current.ng += item.count;
    });
  ...
  return (
    <>
      <style>{`
        ...
        .db-stat:hover { transform: translateY(-3px); box-shadow: 0 8px 24px rgba(0,0,0,0.12); }
        ...
        .db-stat-lbl  { font-size: 12px; font-weight: 500; opacity: 0.92; }
        ...
        .db-progress-fill { height: 100%; background: linear-gradient(90deg, #10b981, #059669); border-radius: 4px; transition: width 0.4s; }
        ...
        .db-page-btn:disabled { background: #9ca3af; cursor: not-allowed; opacity: 0.6; transform: none; }
        ...
        .db-hcard-key { color: #9ca3af; font-weight: 600; text-transform: uppercase; letter-spacing: 0.3px; }
        ...
        @media (max-width: 768px) {
          ...
          /* Switch history to cards on mobile */
        ...
        @media (hover: none) and (pointer: coarse) {
          ...
          .db-month-btn { min-height: 44px; }
          .db-page-btn { min-height: 44px; }
          .db-page-num { min-height: 44px; width: 44px; }
          .db-refresh-btn { min-height: 40px; }
        }
      `}</style>
