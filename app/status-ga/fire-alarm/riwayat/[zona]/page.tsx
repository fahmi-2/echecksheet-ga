...
export default function RiwayatFireAlarm() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { user } = useAuth()

  const zona = searchParams.get("zona") || "zona-1"
    ...
    <div className="app-page">
      ...
      <div className="page-content">
        <div className="header">
          <h1>📍 Riwayat Inspeksi Fire Alarm - {zona.toUpperCase()}</h1>
                    ...
                    <tbody>
                      {record.items.map((item) => (
                        <tr key={`${record.id}-${item.no}`}>
                          ...
                          <td>{item.zona}</td>
                          <td>{item.lokasi}</td>
                          <td>{item.alarmBell}</td>
                          <td>{item.indicatorLamp}</td>
                          <td>{item.manualCallPoint}</td>
                          <td>{item.idZona}</td>
